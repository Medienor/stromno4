...
def get_electricity_api_url():
    ...
    return f"https://www.hvakosterstrommen.no/api/v1/prices/{today}_NO4.json"
...
def get_reservoir_data():
    ...
    if response.status_code == 200:
        data = response.json()
        for item in data:
            if item["omrType"] == "EL" and item["omrnr"] == 4:
                return item
    print(f"Failed to retrieve reservoir data. Status code: {response.status_code}")
    return None

webflow_url = "https://api.webflow.com/v2/collections/66a77de3c69299588f111dfb/items/66a77f438485d6f42aaf176a/live"
...
def main():
    ...
    # Map prices to Webflow fields (rounded to 2 decimal places)
    for item in prices:
